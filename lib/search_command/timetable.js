...
const EkispertClient = require("../ekispert").ekispertClient;

const client = new EkispertClient(process.env.EKISPERT_ACCESS_KEY);
...
async function searchStation() {
  ...
  for (;;) {
    ...
    try {
      summaryResponse = await client.fetchStationSummary(keyWord);
    } catch (e) {
      throw ekispert.getFailureMessage(e);
    }

    if (!ekispert.isNoSearchResult(summaryResponse)) break;
    console.log("検索結果が0件でした。再入力してください。");
  }
...
async function selectRoute(station) {
  const routesResponse = await client.fetchRoutes(station.code);
...
async function displayTimeTable(station, route, options) {
  const timeTableResponse = await client.fetchTimeTable(
    station.code,
    route.code,
    options.daydiagram
  );
  ...
  const header = [
    Object.values(filteredTimeTable.kinds)
      .map((kind) => kind.text("supplement"))
      .join(" "),
    Object.values(filteredTimeTable.destinations)
      .map((destination) => destination.text("supplement"))
      .join(" "),
  ].join("\n");
