...
module.exports = class TimeTableData {
  ...
  range(from = Time.earliestFirstTrainTime, to = Time.latestLastTrainTime) {
    ...
    return new TimeTableData(times, this.line, this.kinds, this.destinations);
  }

  forEachGroupByHour(callback) {
    // ソート済みであることが前提
    let groupTimes = [];
    this.times.forEach((currentTime) => {
  ...
  static createFromResponse(response) {
    ...
    this.kinds = {};
    ekispert
      .normalize(timeTable.LineKind)
      .forEach((k) => (this.kinds[k.code] = new LineKind(k)));

    this.destinations = {};
    ekispert
      .normalize(timeTable.LineDestination)
      .forEach((d) => (this.destinations[d.code] = new LineDestination(d)));
  ...
  static #convertMinuteTableResponseToTimes(minuteTableResponse, hour) {
    ...
    return ekispert.normalize(minuteTableResponse).map((minuteRow) => {
      return new Time(
        hour,
        minuteRow.Minute,
